...
<section class="content">
 ...
 --%>
		<c:choose>
		<c:when test="${(not empty login && login) || (not empty loginError && loginError)}">
		 
			 <form:form class="form"  method="post" action="Login" commandName="userprofile">
			 
<c:set var="anotherTest" value="${userprofile.email}" scope="request"/>
<c:set var="anotherTest" value="${userprofile.password}" scope="request"/>
		         <div class="signR">
		         	
		          <h6>Login with email</h6>
		          
		          <div class="row">
	           <form:input class="single-row"  path="email" placeholder="Email Id" value="" ></form:input> 
	          	<!-- <input type="text" value="Email Id" /> -->
	          </div>
	          <div class="row">
	           <form:input class="single-row"  path="password" placeholder="Password" value="" ></form:input>
